...
- 2026-08-05
  ...
  **[UI]**
  ...
  - Local diagnostic logs — Robot Studio now writes daily files under `~/.robot-studio/logs/` (`frontend-YYYY-MM-DD.log`, `backend-YYYY-MM-DD.log`). File logging runs in release builds too (console stays debug-only). On every startup both sides delete `*.log` files older than **7 days** so support can ask for a recent log without filling the user's disk. Passwords / tokens / file contents stay redacted via the existing body summarizer. Backend request logging records method/path/status/duration (health probes skipped) plus execution lifecycle and unexpected crashes with stacks. Regressions in `backend/tests/test_logging_setup.py`, `test_request_logging.py`, and `frontend/test/app_logger_test.dart`.
...
- 2026-08-06
  ...
  **[UI]**
  ...
  - Package search matches **package names only** and ranks **exact → prefix → substring** (no fuzzy / subsequence matching; summaries ignored). Hyphen/underscore/dot are normalized; results stay deterministic. Applies to **Search installed** and PyPI. PyPI HTML search is bot-blocked, so discovery uses the cached Simple API name index and shows the **top 20** ranked packages (best match first) with JSON metadata for version/summary.
