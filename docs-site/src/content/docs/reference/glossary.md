...
| **Impact Analysis** | Given a keyword at the caret, lists tests that would be affected if it changed (direct / transitive), with confidence. **Run Impact Set** re-runs only the certain (non-low-confidence) hits — see [Impact Analysis](/workflows/impact-analysis/) |
