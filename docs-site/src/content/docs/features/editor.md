...
| Hover | **Robot:** pause on a **keyword** cell for docs/signature. **Python:** pause on a symbol for docs from the stdlib or an installed package |
| Signature help | Shown above the caret line so it never sits under the completion popup. **Robot:** argument hints while the caret is in a keyword call. **Python:** parameter hints while you fill in a call, or from the buffer while you write a `def` header — it does not stay pinned over the function body |
| Breadcrumbs | Path / structure trail above the editor, including the enclosing Robot keyword or Python class/function |
| Go to Definition | F12 or Ctrl/Cmd+Click; multi-match picker when needed. **Python:** follows imports and aliases to where a name is defined; stdlib / site-package sources open in **Peek Definition** (outside the project tree) |
