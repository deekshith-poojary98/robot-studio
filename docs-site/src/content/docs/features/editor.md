...
| Go to Definition | F12, Ctrl/Cmd+Click, or **right-click → Go to Definition**; multi-match picker when needed. Results stay in the **open workspace** — a project you opened earlier is not mixed in. **Python:** follows imports and aliases to where a name is defined; stdlib / site-package sources open in **Peek Definition** (outside the project tree) |
| Context menu | Right-click in the editor for Cut / Copy / Paste, Go to Definition, Peek Definition, Find References, Impact Analysis, Rename Symbol, Format Document, Find, and Replace. Clicking outside an existing selection places the caret first (same as VS Code) |
...
| Diagnostics | Live while editing. **Robot:** library imports resolve via the active environment; keywords defined in a `Resource` you import (and libraries that resource imports) are treated as available (same as Robot Framework); unknown keywords and variables are flagged (`@{list}` / `&{dict}` match the same name imported as `${list}` from a `Variables` Python/YAML file or a `Resource`); **embedded-argument** keywords such as `Login with ${type} credential` match calls like `Login with valid credential`, and `${type}` is treated as declared in that keyword’s body; keyword calls are checked for **unknown**, **missing**, and **extra** arguments when the keyword’s signature is known — including keywords defined in `.resource` / suite files (so a renamed `[Arguments]` name such as `locator` → `selector` warns on leftover `locator=` calls); `name=` cells that do not belong to a wrapper such as `Run Keyword And Return Status` are treated as arguments of the inner keyword (`*args`), not as unknown arguments of the wrapper; unused `Library` / `Resource` imports in the current file are listed as information. Extended variable access like `${response.json()}` is accepted when `${response}` is known. **Python:** syntax errors (all of them, not just the first); lint via the environment’s `ruff check` when installed, otherwise pyflakes (undefined names, unused imports, unused locals); a warning when an `import` / `from` package is not in the active environment; a warning when `from package import Name` refers to a class or function that does not exist on that package; optional **Python member checks** (Settings → Editor) for unknown attributes (`obj.wrong`) and unexpected call keywords (`func(nope=…)`) when Jedi can infer the type |
