...
| Signature help | **Robot:** argument hints while the caret is in a keyword call (also on pointer hover). **Python:** info card appears only when you **hover** a symbol — clicking to place the caret does not open it, and blank parts of a line do not either |
...
| Rename Symbol | **Edit → Rename Symbol…** (also command palette). **Python:** definition and every usage across the project. **Robot:** user keywords (not BuiltIn), including qualified and BDD-prefixed calls |
