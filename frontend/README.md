...
- **Robot Doctor**: activity-rail Project Health Center (`DoctorPage`). Profile Quick/Default/Full; summary shows Findings / Critical / Errors / Warnings (+ plain-language “Since last scan”); optional “lessons from N past runs” when execution knowledge is present. Findings grouped with human category labels (Imports & libraries, Structure, From past runs, …), search/filter/sort, “Why is this reported?”, confidence note, Jump to source. Internal graph-version / linked-run counters are not shown as primary chips. Quick Fix is hidden until real fix handlers ship. Backed by `/api/v1/doctor/*`.
