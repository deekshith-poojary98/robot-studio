...
- **Package search** (**Search installed** and PyPI results) accepts partial matches and ranks **exact → prefix → substring → fuzzy** (ordered subsequence, ≥2 chars). Hyphen/underscore/dot are normalized; ordering is deterministic — no typo engine. PyPI discovery uses the public Simple API name index (cached under `~/.robot-studio/cache/`, refreshed daily) because Warehouse HTML search is bot-gated; the dialog shows the **top 20** ranked hits with versions/summaries from the JSON project API.
