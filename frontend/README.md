...
make package-linux     # → dist/linux/RobotStudio/ (+ linux-x64 or linux-arm64 zip)
```
...
Release builds embed a PyInstaller sidecar under `Contents/Resources/backend/` (macOS) or `backend/` next to the binary (Windows / Linux). `BackendHost` (`lib/core/backend_host.dart`) spawns it when the preferred port (`8765`) is not already healthy, allocating a free nearby port if `8765` is busy, waits on `/api/v1/health`, writes `~/.robot-studio/backend.pid` + `backend.port`, and the native runner (macOS `applicationWillTerminate`, Windows `OnDestroy`, Linux `GApplication::shutdown`) kills that PID on quit — Flutter `detached` alone is not reliable on desktop. Orphan sidecars from a previous crash are reclaimed via the pid/port files. App data stays in `~/.robot-studio` (settings, DB, pid/port files, and daily diagnostic logs under `logs/` — retained for 7 days).
---
...
Plugins remains fully implemented but is **hidden for private beta** from the activity bar
...
**Settings** is available from the **gear at the bottom of the activity bar**,
**File → Settings…**, and the command palette (full center screen — not a dialog).
AI is **not** shipped and has no rail entry.

Activity-bar panels with a side column: Explorer, Search (Find in Files), Libraries,
Tests, Reports. Insights, Packages, Source Control, Doctor, and Settings take over
the main view (the side column collapses when those panels have no side content).
...
- Toolbar: left = project chip (name only; menu: recents, **Open Project**, **Reveal in Finder/Explorer**, **New Project**) / environment / branch (git Fetch·Pull·Push live in a ⋯ menu, repos only); center = command search (⌘⇧P / Ctrl+Shift+P; ⌘K / Ctrl+K still works); right = **run configuration** selector (`Default` until one is chosen) immediately before one attached, equal-width **Run / Project / Stop** segmented control (Run primary, Stop red only while running). The selector does not change Run vs Project semantics or the active toolbar environment; New/Edit/Manage are dialogs (Duplicate is first-class in Manage). Configurations persist at `{project}/.robotstudio/run-configurations.json`. The environment chip stays clickable when it shows **No environment** (Create Environment… / Manage Environments…) so dismissing the missing-env toast does not leave users stuck. No product wordmark (the rail logo carries it), no profile or notification icons. Popup / context menus use compact 28px rows (not Material’s default 48px).
- **Window menu bar** (VS Code–style): **File** (New/Open Project·Workspace, Save/Save All, **Settings…** ⌘,/Ctrl+,, Close/Reopen Editor, Reveal in Finder / File Explorer) · **Edit** (Find/Replace, Find in Project, Format, Word Wrap) · **View** (Command Palette, Go to File, Explorer/Search/Libraries/Insights/Source Control/Tests/Reports/Robot Doctor/Problems, Toggle Side Bar/Terminal) · **Go** (Definition, Peek, References, Go to Symbol in File, Find Symbol in Project, Hover) · **Run** (Run File **F5** / Run Test at Cursor / Run Project / Stop **Shift+F5**) · **Terminal** (Toggle). **Robot Doctor** uses **⌘⇧D** / **Ctrl+Shift+D**. On **macOS** this is the native system menu (plus the **Robot Studio** app menu for About / Hide / Quit). On **Windows / Linux**, the same menus render as an in-window bar under the title. Menu accelerators own those chords on macOS — Flutter `Shortcuts` keeps Ctrl+Tab editor cycling there, and owns the full shortcut map on Windows/Linux.
...
- **Run** is the primary segment in the attached **Run / Project / Stop** toolbar control; Project stays quiet and Stop stays muted when idle. **Run** targets the active `.robot` editor (or the last selected suite when no editor is focused) — never a non-robot file and never a silent fallback to “first suite in the project.” A parent `__init__.robot` Suite Setup is still executed (Robot starts at that folder with `--suite`). A play control in the editor gutter (left of each test/task name in a `.robot` suite) and **Run → Run Test at Cursor** run only that test via `POST /tests/run`. Starting a run brings the **Execution** monitor to the front; Jump to Source brings the editor forward. The **Tests** center view is monitoring-only (live output + recent runs + Failed Tests + status) — it does not duplicate Run File / Run Project / Stop; launch from the toolbar, F5 / Shift+F5, editor gutter, Tests tree actions, menus, or the command palette. When a run has failures, a **Failed Tests** section lists each one with message, **Jump to Source**, and **Re-run Test**. Passing runs and Robot errors with zero failed tests (no matching tags, empty suite) skip that block — no loading skeleton above Live Output. Reports badges follow the same rule: **FAIL** only when tests failed; empty selection is **NO TESTS**.
...
- Reports: opening the page auto-selects the latest run. Rail **Runs** lists recent runs by run number from `.robotstudio/reports/Run-*` (e.g. `Run 20260101-120000 · FAIL`; tap to open details); main Reports page shows dashboard + details only (no duplicate run list). Artifact filenames are hyperlinks; Reveal Folder remains.
...
- **Run** / **Project** stay disabled until Robot Framework is installed in the active environment (tooltip explains why).
- Problems: live diagnostics while editing; missing imports use analysis inspection id `missing_import` (Problems subtitle may show `analysis · missing_import`) — they stay in **Problems**, not Robot Doctor. Click a problem (or status-bar ERRORS/WARNINGS) to jump to file:line:column; **Fix** on a row can install a missing package or insert a `Library` import; panel auto-opens when issues appear and auto-closes when they clear only if it was auto-opened and Problems is still showing (a panel the user opened, or one they switched to Terminal, stays open). The list updates in place while typing — no skeleton flash on every keystroke. Local settings like `[Documentation]` / `[Tags]` are not flagged as unknown keywords. Continuation (`...`), RF automatic variables (`${TRUE}` / `${FALSE}` / …), number variables (`${10}` / `${3.14}` / `${0xFF}`), and variables from `FOR` / `VAR` / `[Arguments]` / assignments are recognized. Missing `Resource` / `Variables` imports are checked on disk and unified with Analysis findings. `Library` imports and their keywords are resolved from the **active environment** (via Robot libdoc), not only the workspace file index — so packages installed in the project venv (e.g. `robotframework-excelsage` → `ExcelSage`) are recognized.
...
- **Parameter authoring:** **Robot** shows a caret-driven signature card while typing keyword arguments (and on pointer hover). **Python** shows the info card only on pointer hover over a symbol or call — the caret alone does not open it. ESC dismisses. Shared source-agnostic `KeywordMetadata` backs signature help, named args, and hover.
...
- Command palette: toolbar search / ⌘⇧P / Ctrl+Shift+P (also ⌘K / Ctrl+K) opens commands + file/symbol search. **Go to File** (`⌘P` / `Ctrl+P`) opens the **same** palette. Sidebar **Search** (or **Edit → Find in Project** / ⌘⇧F / Ctrl+Shift+F) opens **Find in Files** in the left rail while keeping the editor mounted; matches can show enclosing test/keyword/variable labels from the index. Indexed symbol search is **Go → Find Symbol in Project…** (`⌘T` / `Ctrl+T`) or the palette — there is no Symbols activity-bar item.
- **Keyboard shortcuts** (VS Code–aligned where the product supports them):
  - **Window menu bar** owns most chrome chords (Save, Save All, Close, Reopen, Toggle Side Bar/Terminal, Find in Project, Format, Problems, Command Palette / Go to File, **F5** Run File, **Shift+F5** Stop, **⌘/Ctrl+Shift+D** Robot Doctor, **Settings** ⌘,/Ctrl+,) so they don’t double-fire with Flutter `Shortcuts`.
  - **Flutter Shortcuts** still handle Ctrl+Tab / Ctrl+⇧Tab editor cycling (and the full map on Windows/Linux).
  - **Editor** (re_editor + Robot overrides): ⌘/Ctrl+F find · ⌘/Ctrl+H replace · ⌘/Ctrl+/ comment · ⌥/Alt+↑↓ move line · ⇧⌥/Alt+↑↓ copy line · ⌘/Ctrl+⇧K delete line · F12 go to definition · undo/redo. Split editor / multi-cursor are not shipped yet.
- **Tests** rail (`TestExplorerPanel`): lazy suite expand (children load on demand) + virtualized rows; PASS/FAIL/SKIP/NOT RUN/RUNNING; toolbar Run All / Run Current File / Run Failed / Refresh / Expand / Collapse; confirms before large project runs using **Settings → Execution → Large Run Threshold** (default 100; backend 409 uses the same setting); live filter by suite, test, tag, or file. Single-project / standalone opens hide the redundant workspace container row (workspace + project often share the same folder name) and start at the project; multi-project workspaces still show the workspace root.
...
| `editor_run_gutter_test.dart` | Outline → runnable tests; gutter play runs that test and stays inert while a run is active |
