...
- Completions and highlighting separate **RF DSL** (section headers, suite/local settings, `IF`/`FOR`/`TRY`/`VAR`/…) from **BuiltIn library** keywords (`Log`, `Should Be Equal`, … — kind `keyword`, detail BuiltIn). DSL completions use kind `dsl`. Keywords from `Library` imports in the open file are suggested from the **active environment** (libdoc); `WITH NAME` aliases suggest `Alias.Keyword` forms. Popup matching is case-insensitive.
