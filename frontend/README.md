...
- Signature help (bottom-left overlay on keyword calls) loads argument chips + short docs from the **active environment** via Robot libdoc — not only the workspace index — so library keywords like `Open Workbook` show real parameters.
