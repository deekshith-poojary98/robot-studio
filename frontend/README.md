...
- **Run** is the only primary-styled run control; **Run Project** and **Stop** are quiet icon buttons (Stop stays muted when idle). The **Tests** center view is monitoring-only (live output + recent runs + status) — it does not duplicate Run File / Run Project / Stop; launch from the toolbar, F5 / Shift+F5, Test Explorer tree actions, menus, or the command palette.
