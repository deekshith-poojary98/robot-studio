...
class _CodeLineEditingControllerImpl extends ValueNotifier<CodeLineEditingValue>
  ...
  factory _CodeLineEditingControllerImpl.fromText(String? text,
    ...
    return _CodeLineEditingControllerImpl(
        codeLines: text?.codeLines ?? _kInitialCodeLines, options: options);
  }
  ...
  set text(String value) {
    runRevocableOp(() {
      this.value = CodeLineEditingValue(codeLines: value.codeLines);
    });
  }
  ...
  void edit(TextEditingValue newValue) {
    if (newValue.text.isMultiline) {
      ...
      if (beforeText.isNotEmpty && !newValue.text.startsWith(beforeText)) {
        ...
        replacement = newValue.text.substring(
            beforeText.length, newValue.text.length - endText.length);
      }
  ...
  Future<void> copy() {
    if (selection.isCollapsed) {
      // Copy the current line's text only — do not append a trailing linebreak.
      // (Appending one made Ctrl/Cmd+C → V insert an extra blank line after the
      // content when pasting at the caret.)
      return Clipboard.setData(ClipboardData(text: extentLine.text));
    } else {
  ...
  void cut() {
    if (selection.isCollapsed) {
      // Keep the linebreak on cut so paste reinserts a full line after the
      // line is removed (VS Code empty-selection cut).
      Clipboard.setData(
    ...
    if (selection.isCollapsed) {
      ...
      if (selection.extentOffset == extentLine.length) {
        ...
        value = value.copyWith(
            codeLines: newCodeLines,
            selection: CodeLineSelection.collapsed(
                index: selection.extentIndex, offset: selection.extentOffset));
      }
    } else {
      _deleteSelection();
    }
