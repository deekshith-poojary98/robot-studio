...
class _CodeFieldRender extends RenderBox implements MouseTrackerAnnotation {
  ...
  void _onCursorVisibleChanged() {
    _foregroundRender.find<_CodeFieldCursorPainter>().visible =
        _showCursorNotifier.value;
    // Windows Impeller may leave a nested caret layer uncomposited when only
    // the painter notifies. Force a parent paint so the caret appears before
    // the first text layout (keystroke).
    markNeedsPaint();
  }
...
class _CodeFieldExtraRender extends RenderBox {
  ...
  bool get isRepaintBoundary {
    // Windows Impeller (OpenGL ES) does not re-composite this nested layer
    // when only the caret painter marks it dirty. The caret then stays
    // invisible until a parent layout — typically the first keystroke.
