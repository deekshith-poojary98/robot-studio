...
| **Expected**      | Project appears in list/explorer; empty `tests/` / `resources/` / `variables/`; seeded `.gitignore` ignores `.robotstudio/` (envs + reports live there); details show path (no type/template). |
...
| **Expected**      | Opens the **Tests** view with that run output.    |
